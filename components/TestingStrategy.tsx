  ...
  const layers = [
    {
      ...
      details:
        "Mock all external dependencies (DB, Stripe, Email). Focus on edge cases in calculation logic.",
...
describe('PriceCalculator', () => {
  ...
  it('should cap miles if not unlimited plan', () => {
    ...
    },
    {
      ...
      details:
        "Spin up a fresh Postgres container. Execute migration. Run Request. Assert Response. Rollback/Clean.",
...
describe('POST /rentals', () => {
  it('should create rental and lock vehicle', async () => {
    ...
    },
    {
      ...
      details:
        "Use Stripe CLI to trigger events. Verify idempotency keys to prevent double-processing.",
...
describe('Stripe Webhook Handler', () => {
  it('should handle payment_intent.succeeded', async () => {
    ...
    },
    {
      ...
      details:
        "Test database connection pool limits and race conditions during vehicle booking.",
...
export default function () {
  ...
  sleep(1);
    ...
    },
    {
      ...
      details:
        "Ensure atomic transactions work. If Payment fails, Booking must be rolled back.",
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      <div className="flex-shrink-0 mb-6">
        ...
        <p className="text-slate-600 mt-2 max-w-3xl">
          Multi-layered testing architecture ensuring financial accuracy, high
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-6">
        {/* Navigation Sidebar */}
        <div className="w-full md:w-80 flex-shrink-0 flex flex-col gap-3 overflow-y-auto pr-2">
          <div className="p-4 bg-slate-900 rounded-xl text-white mb-2 shadow-lg">
            ...
            <p className="text-xs text-slate-400">
              Standard distribution of test effort.
            </p>

            <div className="mt-4 flex flex-col items-center gap-1">
              <div className="w-16 h-8 bg-orange-500 rounded-t-sm flex items-center justify-center text-[10px] font-bold">
                E2E
              </div>
              <div className="w-32 h-10 bg-emerald-600 flex items-center justify-center text-xs font-bold">
                Integration
              </div>
              <div className="w-48 h-12 bg-blue-600 rounded-b-sm flex items-center justify-center text-sm font-bold">
                Unit
              </div>
            </div>
          </div>
            ...
            >
              <div className="flex items-start justify-between relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`font-semibold ${activeLayer === layer.id ? "text-slate-900" : "text-slate-700"}`}
                  >
                    {layer.title}
                  </span>
                </div>
        ...
        <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="bg-[#0f172a] px-6 py-4 border-b border-slate-800">
            <p className="text-slate-400 text-sm leading-relaxed">
              <span className="text-emerald-400 font-bold uppercase text-xs tracking-wider mr-2">
                Strategy:
              </span>
              ...
              {layers
                ...
                .map((line, i) => (
                  <div key={i} className="table-row">
                    <span className="table-cell select-none text-slate-700 text-right pr-6 w-10 opacity-50">
                      {i + 1}
                    </span>
                        ...
                        if (idx === 0) {
                          return (
                            <span key={idx}>
                              {part
                                .replace(
                                  /(describe|it|expect|test|await|async|const|let|return|import|export|default|function)/g,
                                  '<span class="text-purple-400">$1</span>',
                                )
                                .replace(
                                  /(db\.|request\(|http\.|check\(|sleep\()/g,
                                ...
                                .split(
                                  /<span class="text-purple-400">|<span class="text-blue-400">|<span class="text-amber-300">|<\/span>/,
                                )
                                .map((token, tIdx) => {
                                  if (
                                    part.includes(
                                      `class="text-purple-400">${token}`,
                                    )
                                  )
                                    return (
                                      <span
