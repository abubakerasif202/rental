...
const ApiDocs: React.FC = () => {
      ...
      endpoints: [
        {
          ...
          roles: ["Staff", "Admin"],
          description:
            "Retrieve a paginated list of clients with optional search filters.",
        },
        {
          method: "POST",
          path: "/api/v1/clients",
          ...
          roles: ["Staff", "Admin"],
          description:
            "Register a new client. Performs automatic license validation check.",
          requestBody: {
            firstName: "John",
            lastName: "Doe",
      ...
      endpoints: [
        {
          ...
          roles: ["Public", "Staff"],
          description:
            'Get available vehicles. Public role only sees "Available" status cars.',
        },
        {
          method: "POST",
          path: "/api/v1/vehicles",
      ...
      endpoints: [
        {
          ...
          roles: ["Staff", "Admin"],
          description:
            "Initialize a rental agreement. Locks vehicle availability.",
          requestBody: {
        ...
        },
        {
          ...
          roles: ["Staff", "Admin"],
          description:
            "Finalize a rental upon return. Calculates final fees based on mileage and fuel.",
          requestBody: {
            endMileage: 15600,
            fuelLevel: 100,
      ...
      endpoints: [
        {
          ...
          roles: ["Staff", "Admin"],
          description:
            "Record a maintenance event. Automatically updates vehicle status if needed.",
          requestBody: {
            vehicleId: "uuid-vehicle-5",
  ...
  const getActiveResource = () =>
    resources.find((r) => r.id === activeGroup) || resources[0];

  return (
    <div className="flex flex-col md:flex-row h-full animate-in fade-in duration-500 gap-6">
      {/* Sidebar: Resources */}
      <div className="w-full md:w-64 flex-shrink-0 flex flex-col gap-2">
        ...
        {resources.map((resource) => (
          ...
          >
            <div
              className={`p-2 rounded-lg ${activeGroup === resource.id ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
            >
              <resource.icon className="w-4 h-4" />
            </div>
            <div>
              <span
                className={`block text-sm font-semibold ${activeGroup === resource.id ? "text-slate-900" : ""}`}
              >
                {resource.name}
              </span>
        ...
        <div className="mt-auto p-4 bg-slate-900 rounded-xl text-slate-300">
          ...
          <p className="text-xs text-slate-400 mb-3">
            Download the full collection for testing.
          </p>
            ...
            >
              ...
              >
                <div className="flex items-center gap-4">
                  <span
                    className={`px-3 py-1 rounded-md text-xs font-bold border ${getMethodColor(endpoint.method)}`}
                  >
                    {endpoint.method}
                  </span>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex gap-1">
                    {endpoint.roles.map((role) => (
                      <span
                        key={role}
                        className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] uppercase font-bold tracking-wider rounded border border-slate-200"
                      >
                        {role}
                      </span>
                    ))}
