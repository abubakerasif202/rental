  ...
  const states = [
    {
      ...
      color: "bg-blue-500",
      description:
        "Initial inquiry, availability check, and driver validation.",
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      <div className="flex-shrink-0 mb-6">
        ...
        <p className="text-slate-600 mt-2 max-w-3xl">
          Complete software lifecycle of a rental transaction, mapping business
          rules to database states and logic gates.
        </p>
      </div>

      <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-8">
        {/* Visual Timeline (Left) */}
        <div className="w-full md:w-1/3 overflow-y-auto pr-4 custom-scrollbar">
            ...
            <div className="space-y-6">
              {states.map((state, idx) => (
                ...
                >
                  {/* Node Circle */}
                  <div
                    className={`absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full border-4 border-white shadow-sm flex items-center justify-center transition-colors ${
                      selectedState === state.id ? state.color : "bg-slate-100"
                    }`}
                  >
                    <state.icon
                      className={`w-5 h-5 ${selectedState === state.id ? "text-white" : "text-slate-500"}`}
                    />
                  </div>
                  ...
                  >
                    ...
                    <h3
                      className={`font-bold ${selectedState === state.id ? "text-slate-900" : "text-slate-600"}`}
                    >
                      {state.title}
                    </h3>
        ...
        <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 flex flex-col shadow-2xl overflow-hidden">
                        ...
                        if (idx === 0) {
                          return (
                            <span key={idx}>
                              {part
                                .replace(
                                  /(const|await|if|else|throw|new|for|while|return)/g,
                                ...
                                .split(
                                  /<span class="text-purple-400">|<span class="text-blue-400">|<span class="text-amber-300">|<span class="text-orange-300">|<\/span>/,
                                )
                                .map((token, tIdx) => {
                                  if (
                                    part.includes(
          ...
          <div className="p-4 bg-slate-800 border-t border-slate-700 text-xs text-slate-400 flex items-center gap-2">
            ...
            <span>
              State changes must be wrapped in Database Transactions to ensure
              consistency.
            </span>
          </div>
        </div>
