  ...
  const sections = [
    {
      ...
      content: {
        overview:
          "As a merchant accepting payments, we must adhere to the Payment Card Industry Data Security Standard (PCI-DSS). Our architecture minimizes scope by using SAQ A.",
        requirements: [
...
const handleSubmit = async (event) => {
  ...
  await savePaymentMethodToBackend(paymentMethod.id);
    ...
    },
    {
      ...
      content: {
        overview:
          "We must comply with the Australian Privacy Principles (APPs). This governs how we collect, use, and store Personal Identifiable Information (PII) like Driver Licenses.",
        requirements: [
        ...
        implementation: `// S3 Bucket Policy for Driver Licenses (AES-256 + Private)
{
    ...
    },
    {
      ...
      content: {
        overview:
          "Encryption at rest and in transit is non-negotiable for protecting client data and intellectual property.",
        requirements: [
...
function encrypt(text) {
    ...
    },
    {
      ...
      content: {
        overview:
          "Strict Role-Based Access Control ensures employees only access data necessary for their job function (Principle of Least Privilege).",
        requirements: [
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      <div className="flex-shrink-0 mb-6">
        ...
        <p className="text-slate-600 mt-2 max-w-3xl">
          Regulatory framework and technical implementation guidelines for the
          Australian market.
        </p>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0">
        ...
        <div className="w-full md:w-64 flex-shrink-0 space-y-2">
          {sections.map((section) => (
            ...
            >
              <div
                className={`p-2 rounded-lg ${activeSection === section.id ? "bg-white/10" : "bg-slate-100"}`}
              >
                <section.icon
                  className={`w-5 h-5 ${activeSection === section.id ? "text-emerald-400" : "text-slate-500"}`}
                />
              </div>
              <div>
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs text-blue-700">
                <CheckCircle className="w-3 h-3 text-emerald-500" /> SOC 2 Type
                II (Pending)
              </div>
              <div className="flex items-center gap-2 text-xs text-blue-700">
                <CheckCircle className="w-3 h-3 text-emerald-500" /> ISO 27001
                (Ready)
              </div>
            </div>
          </div>
                  ...
                  <div>
                    ...
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {section.content.requirements.map((req, idx) => (
                        <li
                          key={idx}
                          className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100 text-sm text-slate-700"
                        >
                  ...
                  <div>
                    ...
                    <div className="bg-slate-900 rounded-lg border border-slate-800 overflow-hidden">
                      <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 border-b border-slate-700">
                        ...
                        <span className="text-xs font-mono text-slate-400 ml-2 opacity-50">
