  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
        ...
        <div>
          <h3 className="font-semibold text-blue-900">PostgreSQL 16+ Production Schema</h3>
          <p className="text-sm text-blue-700 mt-1">
            Designed for Multi-tenancy (SaaS), Audit Compliance, and Scalability. 
            Includes UUID keys, RLS-ready structures, JSONB for extensible settings, and strict Foreign Key constraints.
          </p>
        </div>
      </div>
              ...
              <div key={i} className="table-row">
                ...
                <span className="table-cell whitespace-pre-wrap">
                  {line
                    ...
                    .replace(/ENUM/g, '!!ENUM!!')
                    .split('!!').map((part, idx) => {
                      if (part === 'CREATE TABLE' || part === 'ENUM') return <span key={idx} className="text-purple-400 font-bold">{part}</span>;
                      if (part === 'PRIMARY KEY') return <span key={idx} className="text-amber-400">{part}</span>;
                      if (part === 'REFERENCES') return <span key={idx} className="text-blue-400">{part}</span>;
                      return part;
                  })}
                </span>
              </div>
