    ...
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      <div className="flex-shrink-0 mb-6">
        ...
        <p className="text-slate-600 mt-2 max-w-3xl">
          Technical specification for handling high-value rental transactions,
          security deposits, and recurring automated billing.
        </p>
      </div>
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-3 overflow-y-auto pr-2">
            ...
            >
              <div className="flex items-start justify-between relative z-10">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${activeStep === idx ? "bg-white/10 text-emerald-400" : "bg-slate-100 text-slate-500 group-hover:bg-emerald-50 group-hover:text-emerald-600"}`}
                  >
                    <step.icon className="w-5 h-5" />
                  </div>
                  <span
                    className={`font-semibold ${activeStep === idx ? "text-white" : "text-slate-900"}`}
                  >
                    {step.title}
                  </span>
                </div>
              ...
              <p
                className={`mt-3 text-sm leading-relaxed ${activeStep === idx ? "text-slate-400" : "text-slate-500"}`}
              >
                {step.desc}
              </p>
            </button>
          ...
          <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <ul className="list-disc list-inside text-xs text-amber-800 space-y-1">
              <li>
                Always use idempotency keys for charges to prevent double
                billing.
              </li>
              <li>
                Deposits expire after 7 days; implement a refresher job if
                rental is longer.
              </li>
                      ...
                      if (idx === 0) {
                        return (
                          <span key={idx} className="text-slate-300">
                            {part
                              .replace(
                                /(const|await|return|switch|case|break|try|catch|for|if)/g,
